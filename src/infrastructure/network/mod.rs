...
impl<C: Codec> NetworkMiddleware<C> {
    ...
    pub fn new(config: MiddlewareConfig, codec: C) -> Result<Self, MiddlewareError> {
        let backend = match config.backend {
            BackendType::Tokio => {
                Box::new(backends::tokio::TokioTransport::new()?) as Box<dyn NetworkTransport>
            }
            #[cfg(feature = "io-uring")]
            BackendType::IoUring => {
                let io_uring_config = backends::io_uring::IoUringConfig {
                    queue_depth: config.rx_queue_depth.max(config.tx_queue_depth) as u32,
                    buffer_size: config.buffer_size,
                    buffer_pool_size: config.rx_queue_depth,
                    ..Default::default()
                };
                Box::new(backends::io_uring::IoUringTransport::new(io_uring_config)?)
                    as Box<dyn NetworkTransport>
            }
            #[cfg(feature = "dpdk")]
            BackendType::Dpdk => {
                let dpdk_config = backends::dpdk::DpdkConfig {
                    rx_queues: (config.rx_queue_depth / 512).max(1) as u16,
                    tx_queues: (config.tx_queue_depth / 512).max(1) as u16,
                    ..Default::default()
                };
                Box::new(backends::dpdk::DpdkTransport::new(dpdk_config)?) as Box<dyn NetworkTransport>
            }
            #[cfg(feature = "fpga")]
            BackendType::Fpga => {
